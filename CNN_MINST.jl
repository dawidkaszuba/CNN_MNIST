...
# ╔═╡ f0a9fb06-916d-11ef-349e-ed304467756c
begin
	using Flux, MLDatasets, Statistics, Tracker
	using Flux: onehotbatch, onecold, crossentropy, throttle, flatten
...
# ╔═╡ 90d40f38-3703-4f4b-86b5-3a74595ec492
begin
	opt = Adam(0.001)
	batch_size = 32
end

# ╔═╡ 646d1e06-86de-4e50-9790-85c4cec04843
md"""
## Ładowanie danych
"""

# ╔═╡ 04b28c1c-9710-42cb-869b-317a0632c62b
begin
	train_imgs, train_labels = MNIST(:train)[:]  
	test_imgs, test_labels = MNIST(:test)[:]  
end

# ╔═╡ ff77aaf7-61eb-433a-b0fa-8ce6d834dd61

begin
	using Augmentor
	using ImageShow, ImageCore

	img_in = Gray.(train_imgs[:, :, 3])
	...
	mosaicview(
		...
		#augment(img_in, FlipX(0.4)) z prawdopodobieństwem
		...
		# zoom, oba wymiary tak samo
		augment(img_in, Zoom(1.3)),
		#zoom oba wymiary podane jako argumenty
    	augment(img_in, Zoom(1.3, 1.8)),

		#cropowanie
		augment(img_in, Crop(1:28,2:10))
	  
	)
end

# ╔═╡ d354e32a-9bb0-43ef-a4cd-7fb5d394a03a
md"""
## Wyświetlamy jeden losowy obrazek
"""

# ╔═╡ 3e8596d2-bcb7-4d03-b8ff-43a5febfb618
function display_random_image(imgs)
	random_image = rand(1:60000)
	label = train_labels[random_image]
	NROWS, NCOLS = 28, 28
	image_to_display = reshape(Array(imgs[:, :, random_image, 1]), NROWS, NCOLS)
	image_to_display = permutedims(image_to_display, (2, 1))[:, end:-1:1] # obraca o 90 stopni
	heatmap(image_to_display, color=:grays, title="Ktoś chyba chciał napisać: $label")
end

# ╔═╡ cb288050-8149-46c2-ab05-b108fb60f09c
display_random_image(train_imgs)

# ╔═╡ 036a77b0-7d78-4255-9f58-a8e18902cb5e
md"""
## Augmented data
[i tak nikt nie czyta dokumentacji](https://evizero.github.io/Augmentor.jl/stable/operations/)
"""

# ╔═╡ d914f9e3-a0e0-4b43-a730-36828812d56c
md"""
## Jak wyglądają dane
"""

# ╔═╡ aa2159b8-d387-4b4f-89df-947a96a5de51
begin
	println("Images in the train set: $(size(train_imgs))")
	println("Images in the test set: $(size(test_imgs))")
end

# ╔═╡ 5d0d1cc7-d4d4-4473-b828-9c4cb3e9f0a5
begin
	model = Chain(
...
# ╔═╡ 3b660fc5-d7ab-474d-a25d-e3dd26e9c7f6
begin
using Dates
	function reset_params!(model)
    for layer in model.layers
        if layer isa Conv
            layer.weight .= randn(size(layer.weight)) * sqrt(2 / prod(size(layer.weight)[1:3]))
            layer.bias .= 0
        elseif layer isa Dense
            layer.weight .= randn(size(layer.weight)) * sqrt(2 / size(layer.weight, 2))
            layer.bias .= 0
        end
    end
		time = Dates.format(now(), "HH:MM:SS") 
		println("Model zresetowany o: $time")
end
...
# ╔═╡ 601a6348-37ac-43db-94b6-f0ec0f38adda
begin
	function make_minibatches(X, Y, batch_size)
	    ...
	    for i in 1:batch_size:num_samples
	        upper_bound = min(i + batch_size - 1, num_samples)
	        X_batch = X[:, :, :, i:upper_bound]
	        Y_batch = Y[:, i:upper_bound]
	...
	train_imgs_res = reshape(train_imgs, 28, 28, 1, :)
	train_labels_oh_res = onehotbatch(train_labels, 0:9)
	
	
	test_imgs_res = reshape(test_imgs, 28, 28, 1, :)  
	test_labels_oh_res = onehotbatch(test_labels, 0:9)
	
	
	train_set = make_minibatches(train_imgs_res, train_labels_oh_res, batch_size)
	
	train_set_full = (train_imgs_res, train_labels_oh_res)
	
	test_set_full = (test_imgs_res, test_labels_oh_res)	
	
	test_imgs_res = reshape(test_imgs, 28, 28, 1, :)  
	test_labels_oh_res = onehotbatch(test_labels, 0:9)
	
	test_set = make_minibatches(test_imgs_res, test_labels_oh_res, batch_size)
end



# ╔═╡ 2dfc8864-cb9f-44b3-a3b7-411c44229751
begin
	function loss(x, y)
	    model_pred = model(x)
	    return crossentropy(model_pred, y)
	end
		
	function accuracy(x, y)
	    pred = onecold(model(x)) 
	    return mean(pred .== onecold(y)) 
	end
end

# ╔═╡ da015671-8393-4998-a30c-1a3a99cd62e5
begin
	using Flux: params
	...
	for epoch_idx in 1:100
	    ...
	    Flux.train!(loss, params(model), train_set, opt)
						...
						#obrazki          # etykiety
	    acc = accuracy(train_set_full[1], train_set_full[2])
		@info(@sprintf("[%d]: Treningowe accuracy: %.4f", epoch_idx, acc))
		
		acc = accuracy(test_set_full[1], test_set_full[2])
		@info(@sprintf("[%d]: Testowe accuracy: %.4f", epoch_idx, acc))
	
	    if acc >= accuracy_target
			@info("Udało się już osiągnąć cel -> accuracy na poziomie $(accuracy_target*100)%")
	        break
	    end
	
	    if epoch_idx - last_improvement >= 10
	        @warn("Hm...od 10 epok niczego się nie uczę...;(")
	        break
	    end
	end
end	

# ╔═╡ f1dbb6b3-2301-4b57-bdfa-66f842d623be
md"""
**Wysoki recall** – model wychwytuje większość przypadków pozytywnych, ale może mieć więcej fałszywych alarmów.

**Niski recall** – model przegapia wiele rzeczywistych przypadków pozytywnych.

_Recall_ jest szczególnie ważny tam, gdzie pominięcie pozytywnego przypadku ma wysokie koszty lub konsekwencje, np. w wykrywaniu chorób, oszustw czy zagrożeń bezpieczeństwa.

**Wysoka precision** – model przewiduje pozytywne wyniki głównie wtedy, gdy są one prawdziwe, co zmniejsza liczbę fałszywych alarmów.

**Niska precision** – wiele przypadków oznaczonych jako pozytywne jest w rzeczywistości negatywne, co może obniżać użyteczność modelu.

_Precision_ jest ważny tam, gdzie błędne oznaczenia pozytywne mają kosztowne skutki, np. w diagnostyce medycznej, gdzie niechcemy oznaczać zdrowych osób jako chore.
"""

# ╔═╡ a07dd413-d61e-4d75-8e42-f228e0a69933
function evaluate(model, dataset)
    ...
    for (x, y) in dataset
        ...
        total_loss += Flux.Losses.crossentropy(y_pred, y) * size(x, 4)

        pred_labels = onecold(y_pred)
        true_labels = onecold(y)

        correct_predictions += sum(pred_labels .== true_labels)
								# zwraca 1 jeśli na pred lab jest 1 i na true lab 1
        true_positives += sum((pred_labels .== 1) .& (true_labels .== 1))
        false_positives += sum((pred_labels .== 1) .& (true_labels .!= 1))
        false_negatives += sum((pred_labels .!= 1) .& (true_labels .== 1))
    ...
    return test_loss, test_accuracy, precision, recall
end



# ╔═╡ f2d9e4dd-8020-497d-a146-411deef0faed
begin
	test_loss, test_accuracy, precision, recall = evaluate(model, test_set)
...
# ╔═╡ 3c4537d7-1e31-48e7-abe4-625e70ca861e
BSON.@save "/home/dawid/Documents/studia/KONWERSATORIUM NOWOCZESNYCH TECHNIK ANALITYCZNYCH /project/model.bson" model
...
manifest_format = "2.0"
project_hash = "2031f72468d3a54361266e66949cb868b23f1c85"
