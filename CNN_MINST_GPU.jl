...
# ╔═╡ 4d6300c1-d8ec-405f-ab0e-144e7174b00a
CUDA.functional()

# ╔═╡ c6af35c0-88c4-4e07-b5b5-2b7622bd2511
Flux.GPU_BACKEND
...
begin
	...
	train_imgs = gpu(train_imgs)
	train_labels = gpu(train_labels)
	test_imgs = gpu(test_imgs)
...
# ╔═╡ 542c72c4-e252-43fc-afd4-4a73ee6654fd
...
function make_minibatches(X, Y, batch_size)
    # Liczba dostępnych obrazków
    num_samples = size(X, 4)
    minibatches = []

    for i in 1:batch_size:num_samples
        upper_bound = min(i + batch_size - 1, num_samples)
        # Tworzenie batcha na GPU
        X_batch = gpu(X[:, :, :, i:upper_bound])
        Y_batch = gpu(Y[:, i:upper_bound])
...
train_imgs_res = gpu(reshape(train_imgs, 28, 28, 1, :))
train_labels_oh_res = gpu(onehotbatch(train_labels, 0:9))
test_imgs_res = gpu(reshape(test_imgs, 28, 28, 1, :))
test_labels_oh_res = gpu(onehotbatch(test_labels, 0:9))
...
begin
	function loss(x, y)
	    y_hat = model(gpu(x)) 
	    return crossentropy(y_hat, gpu(y))
	end

	function accuracy(x, y)
	    pred = onecold(model(gpu(x))) 
	    return mean(pred .== onecold(gpu(y)))  
	end
...
for epoch_idx in 1:max_epochs
    ...
    acc = accuracy(gpu(train_set_full[1]), gpu(train_set_full[2]))
    @info(@sprintf("[%d]: Train accuracy: %.4f", epoch_idx, acc))
	

    acc = accuracy(gpu(test_set_full[1]), gpu(test_set_full[2]))
...
# ╔═╡ 8ca64527-d1ad-4140-932c-4d9857d560c6
BSON.@save "/home/dawid/Documents/studia/KONWERSATORIUM NOWOCZESNYCH TECHNIK ANALITYCZNYCH /project/model_gpu.bson" model

# ╔═╡ b39f6e61-51de-468c-990a-b46da350c2d9
begin       
	CUDA.reclaim()
end
...
manifest_format = "2.0"
project_hash = "601945f1d7a2be940011de5aeec61c826530a078"
